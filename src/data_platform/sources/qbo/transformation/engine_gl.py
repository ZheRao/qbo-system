"""
src.data_platform.sources.qbo.transformation.engine_gl

Purpose:
    - transform ingested raw GL report from QBO
...
Notes:
    ...
    - warning
        - there are "Beginning Balance" in "Data" nodes, and are being ignored for `fiscal_year` partitioned writes
"""
...
def transform_gl_spark(tasks: list[TaskRecord], scope:range|list[int], spark:SparkSession, path_config:dict) -> SparkDF:
    """
    Purpose:
        - read raw JSON files, prepare tabulated GL report with Spark
    Inputs:
    ...
    raw_path = Path(path_config["root"]) / path_config["bronze"]["gl"]
    ...
    # save
    pl_path = Path(path_config["root"]) / path_config["silver"]["gl"]
    (
        df2
...
def transform_gl_pandas(tasks: list[TaskRecord], scope:range|list[int], path_config:dict) -> pd.DataFrame:
    """
    Purpose:
        - read raw JSON files, prepare tabulated GL report with Pandas
    Inputs:
        - `tasks`: list of dictionaries contain `['company', 'start', 'end']`, each one corresponds to a Spark job (reading file, flatten)
        - `scope`: `range` or list of integers representing fiscal_years for the flatten job
        - `path_config`: the external path config JSON file, dictionary format
    ...
    # saving
    pl_path = Path(path_config["root"]) / path_config["silver"]["gl"] / "pandas"
    for fy, chunk in df[df["fiscal_year"].isin(list(scope))].groupby("fiscal_year", sort=False):
